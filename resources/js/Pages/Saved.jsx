        ...
        <Mobile page={'collection'}>
            ...
            <main>
                ...
                <div className='grid grid-cols-2 lg:grid-cols-4'>
                    { collection.length == 0 ? <p>Sorry you have not saved any recipies yet! <Link className='text-yellow-500' href={route('home')}>Browse</Link></p> : collection.map((item) => <Recipie title={item.title} saved={'true'} id={item.id} sauce={'collection'} img={ JSON.parse(item.images_url) != null ? JSON.parse(item.images_url).urls[0] : 'https://images.unsplash.com/photo-1547592180-85f173990554?q=80&w=1470&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'}></Recipie>) }
                </div>
            </main>
        </Mobile>
    )
